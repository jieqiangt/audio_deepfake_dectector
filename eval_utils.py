...
import numpy as np
import pandas as pd
from plot_utils import create_accuracy_plot, create_confusion_matrix, create_loss_plot, create_roc_curve, create_precision_recall_curve
from sklearn.metrics import precision_recall_curve, roc_curve, auc, f1_score, precision_score, recall_score, det_curve
...
#                 f_res.write(
#                     f'\tEER {attack_type}\t\t= {_eer:8.9f} % (Equal error rate for {
#                         attack_type}\n'
#                 )
...
def compute_eer(target_scores, nontarget_scores):
    ...
    fpr, fnr, thresholds = det_curve(target_scores, nontarget_scores)
    abs_diffs = np.abs(fnr - fpr)
    min_index = np.argmin(abs_diffs)
    eer = np.mean((fnr[min_index], fpr[min_index]))
...
def generate_metrics(batch_size, epochs, learning_rate, train_losses, val_losses, train_acc, val_acc, y_tests, y_pos_probs, total_params,
                     model_name, iteration, device_name, elapsed_time, results_folder):

    # generate eval metrics
    min_train_loss = np.min(train_losses).item()
    min_val_loss = np.min(val_losses).item()

    max_val_acc = np.max(val_acc).item()
    max_train_acc = np.max(train_acc).item()

    max_y_pos_prob = np.max(y_pos_probs).item()
    min_y_pos_prob = np.min(y_pos_probs).item()

    val_precisions, val_recalls, _ = precision_recall_curve(
        y_tests, y_pos_probs)
    val_fprs, val_tprs, _ = roc_curve(y_tests, y_pos_probs)
    roc_auc = auc(val_fprs, val_tprs)

    val_eer, threshold = compute_eer(y_tests, y_pos_probs)
    val_eer = val_eer.item()
    val_preds = [1 if score >= 0.5 else 0 for score in y_pos_probs]
    val_f1 = f1_score(y_tests, val_preds)
    val_recall = precision_score(y_tests, val_preds)
    val_precision = recall_score(y_tests, val_preds)

    # create training evaluation plots
    create_loss_plot(train_losses, val_losses, results_folder)
    create_accuracy_plot(train_acc, val_acc, results_folder)

    # create model evaluation plots
    create_roc_curve(val_fprs, val_tprs, roc_auc, results_folder)
    create_precision_recall_curve(val_recalls, val_precisions, results_folder)
    create_confusion_matrix(y_tests, val_preds, [
                            'bonafide', 'spoof'], results_folder)

    summary_stats = {'model': model_name, 'iteration': iteration, 'gpu': device_name, 'num_params': total_params, 'batch_size': batch_size, 'epochs': epochs, 'learning_rate': learning_rate, 'training_time': elapsed_time, 'min_train_loss': min_train_loss,
                     'min_val_loss': min_val_loss, 'max_train_acc': max_train_acc, 'max_val_acc': max_val_acc, 'max_y_pos_prob': max_y_pos_prob, 'min_y_pos_prob': min_y_pos_prob, 'threshold': threshold, 'f1': val_f1, 'recall': val_recall, 'precision': val_precision, 'eer': val_eer}

    y_probs_df = pd.DataFrame({'y_pos_prob': y_pos_probs, 'y_tests': y_tests})
    y_probs_df.to_csv(f'./results/{results_folder}/y_probs.csv', index=False)

    base_summary = pd.read_csv('./results/run_summary.csv')
    final_summary = pd.concat([base_summary, pd.DataFrame.from_dict(
        summary_stats, orient='index').T], ignore_index=True)
    final_summary.to_csv('./results/run_summary.csv', index=False)
